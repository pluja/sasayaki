...
build: docker-image
	...
	docker run --rm \
		...
		sh -c "./gradlew --no-daemon assembleRelease && chown -Rf $(HOST_UID):$(HOST_GID) /project/app/build /project/app/schemas /project/.gradle-build-cache /project/.android-cache"
